...
def test_002_get_file_location():
    assert DirOps.get_directory_from_file_path(
...
def test_006_exists_folder_where_doesnot_exist():
    assert (
        DirOps.exists_folder(
            folder_path=os.path.join(os.path.dirname(__file__), "no_test_folder")
        )
        is False
    )
...
def test_008_get_all_files_nonrecursive():
    assert (
        DirOps.get_all_files_in_directory(
            folder_path=test_folder, pattern="*.log", recursive=False
        ).sort()
...
def test_009_get_all_files_recursive():
    assert (
        DirOps.get_all_files_in_directory(
            folder_path=test_folder, pattern="*.log", recursive=True
        ).sort()
...
def test_010_get_latest_file():
    assert (
        DirOps.get_latest_file_in_directory(folder_path=test_folder, pattern="*.log")
        == test_folder_file_2
    )
