...
def test_005_text_to_datetime_with_correct_format():
	assert DateOps.text_to_datetime(text=date_str, date_format=date_str_correct_format) == date_datetime
...
def test_007_get_difference_in_year_diff_year():
	assert DateOps.get_difference_in_year(from_date=date_datetime, to_date=future_date_datetime) == 10


def test_008_get_difference_in_year_diff_year_early_month():
	assert DateOps.get_difference_in_year(from_date=date_datetime, to_date=future_date_early_month_datetime) == 9


def test_009_get_difference_in_year_diff_year_same_month_early_day():
	assert DateOps.get_difference_in_year(from_date=date_datetime, to_date=future_date_early_day_datetime) == 9


def test_010_get_difference_in_year_diff_year_pandas_timestamp():
	assert DateOps.get_difference_in_year(from_date=date_datetime, to_date=future_date_pandas_timestamp) == 10


def test_011_get_difference_in_year_diff_year_str_correct_format():
	assert DateOps.get_difference_in_year(from_date=date_datetime, to_date=date_str,
										  date_format=date_str_correct_format) == 0


def test_012_get_difference_in_year_diff_year_str_incorrect_format():
	with pytest.raises(ValueError, match=r'Date should be in the format *'):
		DateOps.get_difference_in_year(from_date=date_datetime, to_date=date_str,
